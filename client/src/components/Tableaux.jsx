// import React from "react";
// import { Tab } from "@headlessui/react";

// function classNames(...classes) {
//   return classes.filter(Boolean).join(" ");
// }

// export default function Tableaux({ tableaux, setSelected, children }) {
//   return (
//     <div className="w-full px-1 sm:px-0">
//       <Tab.Group>
//         <Tab.List className="flex space-x-6 rounded-xl p-1">
//           {tableaux.map((tableau, index) => (
//             <Tab
//               key={tableau.title}
//               onClick={() => setSelected(index)}
//               className={({ selected }) =>
//                 classNames(
//                   "w-fit flex items-center outline-none gap-2 px-3 py-2.5 text-base font-medium leading-5 bg-white",
//                   selected
//                     ? "text-blue-700 border-b-2 border-blue-600"
//                     : "text-gray-800 hover:text-blue-800"
//                 )
//               }
//             >
//               {tableau.icon}
//               <span>{tableau.title}</span>
//             </Tab>
//           ))}
//         </Tab.List>
//         {/* <Tab.Panel className="w-full mt-2">{children}</Tab.Panel> */}
//         {tableaux.map((_, index) => (
//   <Tab.Panel key={index} className="w-full mt-2">
//     {index === 0 ? children[0] : children[1]}
//   </Tab.Panel>
// ))}

//       </Tab.Group>
//     </div>
//   );
// }
import React from "react";
import { Tab } from "@headlessui/react";

function classNames(...classes) {
  return classes.filter(Boolean).join(" ");
}

export default function Tableaux({ tableaux, setSelected, children }) {
  return (
    <div className="w-full px-1 sm:px-0">
      <Tab.Group onChange={setSelected}>
        <Tab.List className="flex space-x-6 rounded-xl p-1">
          {tableaux.map((tableau, index) => (
            <Tab
              key={tableau.title}
              className={({ selected }) =>
                classNames(
                  "w-fit flex items-center outline-none gap-2 px-3 py-2.5 text-base font-medium leading-5 bg-white",
                  selected
                    ? "text-blue-700 border-b-2 border-blue-600"
                    : "text-gray-800 hover:text-blue-800"
                )
              }
            >
              {tableau.icon}
              <span>{tableau.title}</span>
            </Tab>
          ))}
        </Tab.List>

        {/* Render the selected tab's content */}
        <Tab.Panels className="mt-2">
          {tableaux.map((_, index) => (
            <Tab.Panel key={index} className="w-full">
              {children[index]} {/* Dynamically render the corresponding child */}
            </Tab.Panel>
          ))}
        </Tab.Panels>
      </Tab.Group>
    </div>
  );
}
